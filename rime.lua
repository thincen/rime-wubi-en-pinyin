...
function calculator(input, seg)
    if string.find(input, 'cococo') ~= nil then -- 匹配 cococo 开头的字符串
        local _, _, a, operation, b = string.find(input, "cococo(%d+%.?%d*)([%+%-%*/])(%d+%.?%d*)")
        local result = 0
        if operation == '+' then
            result = a + b
        ...
        yield(Candidate("cococo", seg.start, seg._end, result, "计算器"))
    end
end
...
function date_translator(input, seg)
    ...
    if (input == "timestamp") then
        yield(Candidate("timestamp", seg.start, seg._end, string.format('%d', os.time()), ""))
    end
end
